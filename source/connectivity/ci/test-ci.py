...
class Tests(unittest.TestCase):
    def test_real_bundle_and_corruption(self):
        original = APP / 'modules/6.18.39-haos/npu'
        bundle.verify(original)
        with tempfile.TemporaryDirectory() as tmp:
            dest = Path(tmp) / '6.18.39-haos/npu'
    ...
    def test_wrong_kernel_elf(self):
        with tempfile.TemporaryDirectory() as tmp:
            dest = Path(tmp) / '6.18.52-haos/npu'
            shutil.copytree(APP / 'modules/6.18.39-haos/npu', dest)
    ...
    def test_release_requires_otp(self):
        with tempfile.TemporaryDirectory() as tmp:
            app = Path(tmp)
            root = app / 'modules/6.18.39-haos'
            shutil.copytree(APP / 'modules/6.18.39-haos', root)
    ...
    def test_runtime_selects_kernel_without_fallback(self):
        original = APP / 'modules/6.18.39-haos/npu/bundle.json'
        data = json.loads(original.read_text())
        with patch.object(vendor.os, 'uname', return_value=types.SimpleNamespace(release='6.18.52-haos')):
            ...
            data['kernel_release'] = '6.18.52-haos'
...
def main():
    ...
    print('CI_CONTRACT_PASS tests=8 mutations=9 no_publication_no_board_access reports_only_artifacts')
